...
自己是一个喜欢折腾的工程师，致力于全栈开发。技术栈：vue+vuex+ssr+jq+koa+thinkjs+nginx+redis+websocket+mysql+小程序等
...
将数据层和组建层抽离出来，吧数据层放到全局形成单一的Store，组建变得更薄，专门用来数据展示和操作。
